...
def tombol(source, side, text, command=None, bg="black"):
...
class app(Frame):
    def __init__(self):
        ...
        display = StringVar()
        Entry(self, relief=FLAT, textvariable=display, bd=30, bg="black").pack(
        ...
        for TombAngka in ("789", "456", "123", "0.+", "/*-"):
            ...
            for isdm in TombAngka:
                tombol(
                    ...
                    LEFT,
                    isdm,
                    lambda sObj=display, q=isdm: sObj.set(sObj.get() + q),
                )
        ...
        for TombolC in ["C"]:
            ...
            for ichar in TombolC:
                tombol(
                    ...
                    RIGHT,
                    ichar,
                    lambda sObj=display, q=ichar: sObj.set(""),
                    bg="red",
                )
        ...
        for sd in "=":
            if sd == "=":
                ...
                TombolSama.bind(
                    "<ButtonRelease-1>",
                    lambda e, s=self, sObj=display: s.hitung(sObj),
                    "+",
                )
            else:
                TombolSama = tombol(
